...
def solicitar_exames(request):
    tipos_exames = TiposExames.objects.all() #isso serve para processar todos os dados da models
    
    if request.method == "GET":
        return render(request, 'solicitar_exames.html', {'tipos_exames': tipos_exames})
    elif request.method == "POST":
        exames_id = request.POST.getlist('exames') #getlist serve para pegar todos os ids
...
def fechar_pedido(request):
    ...
    solicitacao_exames = TiposExames.objects.filter(id__in=exames_id) #saber quantos exames precisam ser salvos
    ...
    pedido_exame.save() #isso que faz salvar no banco de dados
